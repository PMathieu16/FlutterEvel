import 'package:flutter/material.dart';
import 'package:shared_preferences/shared_preferences.dart';

class AuthService extends ChangeNotifier {
  static init() async {
    _prefs = await SharedPreferences.getInstance();
  }

  static late final SharedPreferences _prefs;

  Future<void> loginUser(String userName, String userPassword) async {
    try {
      _prefs.setString('userName', userName);
      _prefs.setString('userPassword', userPassword);
    } catch (e) {
      print(e);
    }
  }

  Future<bool> isLoggedIn() async {
    String? userName = await _prefs.getString('userName');
    String? userPassword = await _prefs.getString('userPassword');
    if (userName == null && userPassword == null) return false;
    return true;
  }

  void logoutUser() {
    _prefs.clear();
  }

  String? getUserName() {
    return _prefs.getString('userName') ?? 'DefaultValue';
  }

  String? getUserPassword() {
    return _prefs.getString('userPassword') ?? 'DefaultValue';
  }

  void updateUserName(String newName) {
    _prefs.setString('userName', newName);
    notifyListeners();
  }
}
